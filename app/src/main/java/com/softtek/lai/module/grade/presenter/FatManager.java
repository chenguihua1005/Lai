...
public class FatManager {
    ...
    public FatManager(StudentListCallback cb) {
        this.cb = cb;
        service= ZillaApi.NormalRestAdapter.create(GradeService.class);
        token= UserInfoModel.getInstance().getToken();
    }

    public void loadFat(String classId){
        String userId= UserInfoModel.getInstance().getUser().getUserid();
        service.getStudentsList(token,Long.parseLong(userId),classId,"2", new Callback<ResponseData<List<StudentModel>>>() {
            @Override
