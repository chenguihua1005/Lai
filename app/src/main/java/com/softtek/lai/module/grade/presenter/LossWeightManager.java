 ...
 */
public class LossWeightManager {

    private GradeService service;
    private StudentListCallback cb;
    private String token;

    public LossWeightManager(StudentListCallback cb) {
        this.cb = cb;
        service= ZillaApi.NormalRestAdapter.create(GradeService.class);
        token= UserInfoModel.getInstance().getToken();
    }

    public void loadLossWeight(String classId){
        String userId= UserInfoModel.getInstance().getUser().getUserid();
        service.getStudentsList(token,Long.parseLong(userId),classId,"0", new Callback<ResponseData<List<StudentModel>>>() {
            @Override
