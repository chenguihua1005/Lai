 ...
 */
public class LossWeightPerManager {

    private GradeService service;
    private StudentListCallback cb;
    String token;

    public LossWeightPerManager(StudentListCallback cb) {
        this.cb = cb;
        service= ZillaApi.NormalRestAdapter.create(GradeService.class);
        token= UserInfoModel.getInstance().getToken();
    }

    public void loadLossWeightPer(String classId){
        String userId= UserInfoModel.getInstance().getUser().getUserid();
        service.getStudentsList(token,Long.parseLong(userId),classId,"1", new Callback<ResponseData<List<StudentModel>>>() {
            @Override
