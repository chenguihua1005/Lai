...
public class UnMeasureFragment extends LazyBaseFragment<MesurePresenter> implements AdapterView.OnItemClickListener, PullToRefreshBase.OnRefreshListener2<ListView>, MesurePresenter.MesureView {
    ...
    public static Fragment getInstance(String Classid) {
        UnMeasureFragment fragment = new UnMeasureFragment();
    ...
    public void getMesureList(List<MeasureListModel> list) {
        if (list.size() != 0) {
            memberModels.addAll(list.get(0).getMemberList());
